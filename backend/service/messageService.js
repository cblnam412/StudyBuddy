import { Message, RoomUser, Room, Document } from "../models/index.js";
...
export class MessageService {
    ...
    async detectArchivedRoom(roomId) {
        ...
        if (room.status === "archived") throw new Error("Phòng đang ở trạng thái lưu trữ.");
        return true;
    }
    ...
    async sendMessage(roomId, userId, content, replyTo = null, eventId = null, documentId = null) {
        ...
        try { await this.handlerChain.handle({ message: content }, userId); } catch (error) { throw error; }

        const messageData = { room_id: roomId, user_id: userId, content, reply_to: replyTo };
        if (eventId) messageData.event_id = eventId;
        if (documentId) messageData.document_id = documentId;
        ...
        if (this.smartAI) {
            try { this.smartAI.runAICheckBackground(content, userId, newMessage._id); }
            catch (err) { console.error('[SmartAI] Background error:', err); }
        }
        return populated;
    }
    ...
    async getLastMessagesFromUserRooms(userId) {
        ...
        const lastMessages = await Promise.all(
            roomIds.map(async (roomId) => {
                const lastMsg = await this.Message.findOne({ room_id: roomId, status: { $ne: "deleted" } })
                    .populate('user_id', 'full_name avatarUrl')
                    .sort({ created_at: -1 })
                    .lean();
                return { room_id: roomId, last_message: lastMsg || null };
            })
        );
        return lastMessages;
    }
    ...
    async getMessageById(messageId) {
        ...
        if (!mongoose.isValidObjectId(messageId)) throw new Error("messageId không hợp lệ.");
    ...
    async searchInRoom(roomId, userId, keyword) {
        ...
        // 2. Tìm tin nhắn
        const messagesPromise = this.Message.find({
            room_id: roomId,
            content: { $regex: regex },
            status: { $ne: "deleted" }
        })
        ...
        // 3. Tìm tài liệu (Nếu chưa cần thì để trống mảng này cũng được, nhưng cứ để code chạy sẵn)
        ...
        const [messages, documents] = await Promise.all([messagesPromise, documentsPromise]);
